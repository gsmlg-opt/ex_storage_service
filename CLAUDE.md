...
Requests flow through: `assign_request_id` → `check_presigned_auth` → `RateLimiter` → route match → `Handlers`/`MultipartHandlers` → `Storage.Engine` (disk) + `Metadata` (Concord) → XML response.
