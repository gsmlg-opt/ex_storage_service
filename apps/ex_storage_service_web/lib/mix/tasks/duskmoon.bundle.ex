defmodule Mix.Tasks.Duskmoon.Bundle do
  ...
  Elements are registered as custom elements (e.g. <el-dm-button>).
  Each element's register.js is self-registering via customElements.define().
  """
  ...
  def run(args) do
    ...
    # Create a virtual entry that imports each element's register
    imports =
      Enum.map_join(elements, "\n", fn el ->
        "import '@duskmoon-dev/#{el}/register';"
      end)

    # Write temp entry file inside project (OXC requires entry inside cwd)
    ...
    case OXC.bundle(entry,
      ...
      {:ok, %{code: code}} ->
        write_output(output_path, code, elements)

      {:ok, code} when is_binary(code) ->
        write_output(output_path, code, elements)

      {:error, errors} ->
        Mix.raise("Failed to bundle elements: #{inspect(errors)}")
    end
  after
    tmp_dir = Path.join(File.cwd!(), "_build/duskmoon_bundle")
    File.rm_rf(tmp_dir)
